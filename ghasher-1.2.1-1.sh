...
cygdoc() {
	...
	for f in ${install_docs} ; do
		if [ -f ${S}/${f} ] ; then
			${INSTALL_DATA} ${S}/${f} ${D}/usr/share/doc/${P1} ;
		elif [ -f ${S}/${f}.txt ] ; then
			${INSTALL_DATA} ${S}/${f}.txt ${D}/usr/share/doc/${P1} ;
		fi
	done
	...
	for f in ${install_html} ; do
		if [ -f ${S}/${f} ] ; then
			...
			${INSTALL_DATA} ${S}/${f} ${D}/usr/share/doc/${P1}/html ;
		fi
	done

	if [ -f ${C}/README ] ; then
		${INSTALL_DATA} ${C}/README ${D}/usr/share/doc/Cygwin/${P1}.README ;
	fi

	for f in ${PN1} ${PN2} ${PN3} ${PN4} ; do
		if [ -f ${C}/${f}.README ] ; then
			${INSTALL_DATA} ${C}/${f}.README ${D}/usr/share/doc/Cygwin/${f}.README ;
		fi ;
	done
}
